...
def metric(pred, true, mean, std, metrics=['mae', 'mse'],
    ...
    print(pred.shape,"prdshape")
    sample=pred[0]
    sample_true=true[0]
    print(pred.shape)
    for i in range(11):
        cv2.imwrite('Predictions/pred/image_%d_pred.jpg'%(i),255*sample[i][0])
        cv2.imwrite('Predictions/true/image_%d_true.jpg'%(i),255*sample_true[i][0])
    if 'ssim' in metrics:
        ssim = 0
        for b in range(pred.shape[0]):
            for f in range(pred.shape[1]):
                ssim += cal_ssim(pred[b, f].swapaxes(0, 2).swapaxes(0, 1),
                                 true[b, f].swapaxes(0, 2).swapaxes(0, 1), multichannel=True,channel_axis=2,data_range=1)
