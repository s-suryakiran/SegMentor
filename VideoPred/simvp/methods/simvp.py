...
from simvp.datasets.dataloader_shapes import load_data
...
#             print(len(batch_x))
    ...
    def test_one_epoch(self, test_loader, **kwargs):
        ...
        uid = kwargs['testdir'].strip().split('/')[-1]
        try:
            os.mkdir('./%s_outputs'%(uid))
        except:
            print('./%s_outputs already exists.'%(uid))
                  
        for fold in root_dir:
            data_dir = '%s/%s'%(root,fold) #each folder load
            _,_,newtest = load_data(1,1,data_dir,mode = 'test')
            
            test_pbar = tqdm(newtest)
            ...
            try:
                os.mkdir('./%s_outputs/%s'%(uid,fold))
            except:
                print('./%s_outputs/%s already exists.'%(uid,fold))

            for i in range(11):
                img = pred_y_np[0][i].swapaxes(0, 2).swapaxes(0, 1)
                temp =Image.fromarray((255*img).astype(np.uint8))
                temp.save('./%s_outputs/%s/%d.png'%(uid,fold,i))
                np.save('./%s_outputs/%s/%d.np'%(uid,fold,i),img)
            sys.exit()
